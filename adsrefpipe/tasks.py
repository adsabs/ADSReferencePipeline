...
def task_process_reference_file(reference_filename):
    ...
    for result in results:
        ...
        for batch in range(0, len(references), num_references):
            ...
            resolved_batch = utils.resolve_references(type, references_batch)
...
def task_reprocess_subset_references(record):
    ...
    type = utils.ReferenceType.text if record['parser'] == 'Text' else utils.ReferenceType.xml
    ...
    for batch in range(0, len(references), num_references):
        ...
        references_batch = references[batch: batch+num_references]
        # accumulate them to be inserted to db in once chunk
        resolved += utils.resolve_references(type, references_batch)
        if not resolved:
            return False
    classic_resolved_filename = reference_filename.replace('sources', 'resolved') + '.result' if config['COMPARE_CLASSIC'] else None
